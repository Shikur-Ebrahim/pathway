        ...
        {appStatus === 'accepted' ? (
          showInterviewPicker ? (
            <div className="px-5 py-5 border-t border-gray-100 shrink-0 space-y-4">
              ...
              {emailError && <p className="text-[12px] text-red-500 font-semibold">❌ {String(emailError).includes('API key') || String(emailError).includes('unauthorized') ? 'API key missing. Add RESEND_API_KEY in Vercel settings.' : String(emailError)}</p>}
